...
export async function GET(req: NextRequest, ctx: { params: Promise<{ ma: string }> }) {
  ...
  const chu = await mot(`select n.*, c.slug, c.cookie_ngay, c.cap_click_ngay, c.diem_click from nguoi_tham_gia n join chien_dich c on c.id=n.chien_dich_id where n.ma=$1`, [ma]);
  ...
  // Điểm click thật quay lại từ kênh share (trần theo ngày, không tính người tự bấm link mình)
  if (kenh && ip !== chu.ip) {
