...
export async function POST(req: NextRequest) {
  const { ma, kenh } = await req.json();
  if (!ma || !kenh || String(kenh).length > 20) return NextResponse.json({ congDiem: false });
  const ng = await mot(
    `select n.id, n.xac_minh, n.chien_dich_id, c.diem_share, c.kenh_share from nguoi_tham_gia n join chien_dich c on c.id=n.chien_dich_id where n.ma=$1`,
    [ma]
  );
  if (!ng || !ng.xac_minh || !ng.kenh_share.split(",").includes(kenh)) return NextResponse.json({ congDiem: false });
